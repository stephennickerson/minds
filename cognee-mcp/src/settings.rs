...
pub(crate) struct Settings {
    ...
    pub(crate) read_model_path: PathBuf,
...
fn read_model_path() -> PathBuf {
    env_value("COGNEE_MCP_READ_MODEL_PATH")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(".cognee").join("read_model.sqlite"))
}
